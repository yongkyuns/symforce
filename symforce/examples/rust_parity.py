...
EXAMPLES = (
    ...
    Example(
        ...
        "rust_bundle_adjustment_reference",
        BA_MANIFEST,
        ("runtime",),
        True,
    ),
...
def run(
    command: list[str], cwd: Path, evidence: Path, name: str, input_text: str | None = None
) -> str:
...
def main() -> int:
    ...
    for example in EXAMPLES:
        try:
            ...
            cpp = parse_metrics(
                run(
                    [str(binary), *example.cpp_args],
                    root,
                    evidence,
                    example.name + "_cpp",
                    fixture if example.uses_ba_fixture else None,
                ),
                example.name,
            )
            rust = parse_metrics(
